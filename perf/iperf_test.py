...
class iperf:
	def __init__(self,optionlist):
		...
		self.iperf_command = self.iperf3_command; # Run iperf3 by default
		...
		for i in np.arange(0,len(optionlist)):
			if optionlist[i] == "-r": # bidirectional run. iperf only
				...
				if ((optionlist[i+1] == "iperf2") or\
					...
					self.iperf_command = self.iperf2_command;
				elif (optionlist[i+1] == "iperf3"):
					self.iperf_command = self.iperf3_command;
				else:
					print("unsupported iperf command type. specify either 'iperf2'");
					print("or 'iperf3' as a parameter for -g. Exiting");
					exit(-1);
			elif optionlist[i] == "-e":
				self.option_map[optionlist[i]] = str(optionlist[i+1]);
...
# Given a lower and upper bound, find all the values between the two.                        
	...
	def get_avg_bw_iperf(self,line):
		columns = line.split('  '); # split columns
		# bw data in last column
		bw_string = columns[len(columns)-1].split(' ');
		notation = bw_string[len(bw_string)-1].split('bits/sec');# get format
		if (notation[0] == 'K'):
			factor = 1024.0;
		elif (notation[0] == 'M'):
		...
		sent_rate = float(factor*float(bw_string[len(bw_string)-2]));
		# recv and sent rates are not available separately
		rcv_rate = sent_rate;

		return sent_rate,rcv_rate;

	def get_avg_bw(self,outfile,num_streams=0):
		if (self.iperf_command == self.iperf3_command):
			f = open(outfile, 'r'); # json format
			...
			duration = str(self.option_map["-t"][0]);
			...
			for line in f.readlines():
				if (self.roption == True): # bidirectional.
					...
					if total_duration in line:
						...
						break;
				elif '[SUM]' in line: # look for summation if more than 1 parallel stream
					if total_duration in line: # total summation
						sent_rate,rcv_rate = self.get_avg_bw_iperf(line);
						break;
...
#	test_iperf3("-h");
	...
	if (len(sys.argv) == 2):
	   [optionlist,gw_info] = get_config(sys.argv[1]);
	    ...
	    print("Enter a space separated list of options for the iperf command\n");
	    optionlist = input().split(' ');

	    print("Enter the purpose of using the iperf command: Options:VPN/Other");
	    purpose_option = input();
	    if (purpose_option == "VPN"):
	...
	ipf = iperf(optionlist);
	ipf.print_option_map();

	ipf.execute_test(gw_info,optionlist);
